...
function Gimnasio() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Variantes comunes del nombre, evitando confusión con "gimnasia"
        const TARGETS = ["gimnasio", "musculacion", "sala de musculacion", "fitness"];

        const match =
          sports.find((sport) => {
            const n = normalize(sport?.name);
            const isTarget = TARGETS.some((k) => n.includes(normalize(k)));
            const isGimnasia = n.includes("gimnasia"); // evita artistica/acuatica
            return isTarget && !isGimnasia;
          }) || null;
  ...
  return (
    <main className="mainSports" style={{ marginTop: "8rem" }}>
      <img src={gimnasioPort} className="portadaBs col-lg-12" alt="portada-gimnasio" />
