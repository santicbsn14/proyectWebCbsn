...
function Natacion() {
  ...
  // IDs únicos para evitar colisiones con otros acordeones
  const ACC_ID = "accordion-natacion";
  const collapseId = (i) => `natacion-collapse-${i}`;
  const headingId  = (i) => `natacion-heading-${i}`;
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Variantes comunes del nombre (evitamos cruzarnos con "Gimnasia Acuática / Acuagym")
        const TARGETS = ["natacion", "natatorio"];
        const match =
          sports.find((sport) => {
            const n = normalize(sport?.name);
            const isTarget = TARGETS.some((k) => n.includes(k));
            const isAcuatica = n.includes("acuatica") || n.includes("acuagym") || n.includes("aquagym");
            return isTarget && !isAcuatica;
          }) || null;
  ...
  return (
    <main className="mainSports" style={{ marginTop: "8rem" }}>
      <img src={natatorioPort} className="portadaBs col-lg-12" alt="portada-natacion" />
