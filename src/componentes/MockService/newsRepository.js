export const getNews = async () => {
  try {
      const response = await fetch('https://apicbsn-dev-krgf.2.us-1.fl0.io/api/news?limit=4');
      if (!response.ok) {
...
{
  try {
    const response = await fetch(`https://apicbsn-dev-krgf.2.us-1.fl0.io/api/news${id}`);
    if (!response.ok) {
        throw new Error(`Network response was not ok: ${response.status}`);
    }
    return response.json(); // Retorna la respuesta como JSON
...
export const createNews = async (news) =>
{
    try
    {
        const response = await fetch ('https://apicbsn-dev-krgf.2.us-1.fl0.io/api/news', {
            method: 'POST',
            headers: {
